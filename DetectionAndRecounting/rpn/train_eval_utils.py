...
from rpn.coco_utils import get_coco_api_from_dataset
from rpn.coco_eval import CocoEvaluator
import time
...
def train_one_epoch(model, optimizer, data_loader, device, epoch, print_freq,
    ...
    start = time.clock()
    i = 0
    for images, images1, targets, targets1 in enumerate(data_loader):
        ...
        loss_dict = model(images, targets, images1, targets1)
...
def evaluate(model, data_loader, device, mAP_list=None):
    ...
    for image, image1, targets, targets1 in metric_logger.log_every(data_loader, 100, header):
        image = list(img.to(device) for img in image)
        targets = [{k: v.to(device) for k, v in t.items()} for t in targets]
        image1 = list(img.to(device) for img in image1)
        targets1 = [{k: v.to(device) for k, v in t.items()} for t in targets1]
        # 当使用CPU时，跳过GPU相关指令
        ...
        outputs = model(image, targets, image1, targets1)
