...
class SSD(nn.Module):
    ...
    def __init__(self, phase, size, base, extras, head, num_classes):
        ...
        self.gif=nn.Sequential(
            nn.ModuleList(gif(512)),
            nn.ModuleList(gif(1024)),
            nn.ModuleList(gif(512)),
            nn.ModuleList(gif(256)),
            nn.ModuleList(gif(256)),
            nn.ModuleList(gif(256)),
            nn.ModuleList(gif(256))
        )
    ...
    def forward(self, x,y):
        ...
        for k,v in enumerate(self.extras_1):
            y = F.relu(v(y),inplace=True)
            if k % 2 == 1:
                sources_2.append(y)
        for i in range(len(sources_1)):
            cat=deal(sources_1[i],sources_2[i],self.gif[i+2])
            sources.append(cat)
        ...
        if self.phase == "test":
            output = self.detect(
                loc.view(loc.size(0), -1, 4+4)[...,:-4],                   # loc preds
...
def gif(in_channels):
    layers=[]
    layers+=[nn.Conv2d(2 * in_channels, 1, (3, 3),padding=1)]
    layers+=[nn.Conv2d(2 * in_channels, 1, (3, 3),padding=1)]
    layers+=[nn.Conv2d(2 * in_channels, 2 * in_channels, (1, 1)),nn.ReLU(inplace=True)]
    return layers

def deal(F1,F2,net):
    F_G = torch.cat((F1, F2), 1)
    temp1=net[0](F_G)
    temp2=net[1](F_G)
    m=nn.Sigmoid()
    w1 = m(temp1)
    ...
    for i in range(F1.shape[1]):
        ...
        sum_1 = t1 * w1
        sum_2 = t2 * w2
        t3 = torch.cat((sum_1, sum_2), 1)
        if i==0:
            result=t3
        else:
            result = torch.cat((t3, result), 1)
    result=net[2](result)
    return result
...
def multibox(vgg, extra_layers, cfg, num_classes):
    ...
    for k, v in enumerate(vgg_source):
        loc_layers += [nn.Conv2d(vgg[v].out_channels*2,
                                 cfg[k] * (4+4), kernel_size=3, padding=1)]
        conf_layers += [nn.Conv2d(vgg[v].out_channels*2,
                        cfg[k] * num_classes, kernel_size=3, padding=1)]
    for k, v in enumerate(extra_layers[1::2], 2):
        loc_layers += [nn.Conv2d(v.out_channels*2, cfg[k]
                                 * (4+4), kernel_size=3, padding=1)]
        conf_layers += [nn.Conv2d(v.out_channels*2, cfg[k]
